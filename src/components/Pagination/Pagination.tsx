...
/**
range = 2 áp dụng cho khoảng cách đầu, cuối và xung quanh current_page
...
export default function Pagination({ queryConfig, pageSize }: Props) {
  ...
  const renderDotAfter = (index: number) => {
    if (!dotAfter) {
      dotAfter = true
      return (
        <span key={index} className='mx-2 mb-1 rounded border bg-white px-3 py-2 shadow-sm'>
          ...
        </span>
      )
    }
      ...
      .map((_, index) => {
        ...
        return (
          <Link
            ...
            key={index}
            className={classNames('mx-2 mb-1 cursor-pointer rounded border bg-white px-3 py-2 shadow-sm', {
